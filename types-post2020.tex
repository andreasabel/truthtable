...
\Copyright{Andreas Martin Abel} %TODO mandatory, please use full first names. LIPIcs license is "CC-BY";  http://creativecommons.org/licenses/by/3.0/
...
\supplement{\url{https://github.com/andreasabel/truthtable}}%optional, e.g. related research data, source code, ... hosted on a repository like zenodo, figshare, GitHub, ...

\funding{This work has been supported by grant no.~2019-04216 \emph{Modal
  Dependent Type Theory} of the Swedish Research Council
(Vetenskapsrådet).}%optional, to capture a funding statement, which applies to all authors. Please enter author specific funding statements as fifth argument of the \author macro.
...
I am also grateful for the feedback of the reviewer that led to a
substantial clarification of the proof using orthogonality.
}%optional
...
\EventEditors{Ugo de'Liguoro, Stefano Berardi, and Thorsten Altenkirch}
\EventNoEds{3}
\EventLongTitle{26th International Conference on Types for Proofs and Programs (TYPES 2020)}
\EventShortTitle{TYPES 2020}
\EventAcronym{TYPES}
\EventYear{2020}
\EventDate{March 2--5, 2020}
\EventLocation{University of Turin, Italy}
\EventLogo{}
\SeriesVolume{188}
\ArticleNo{4}
...
Finally, in the \Cref{sec:sat}, we demonstrate how the the saturated sets
...
This repairs the proof; in \Cref{sec:satbeta} we will see a variant of
...
\begin{proof}
  ...
  % (Alternatively, we could induct on
...
\end{proof}
Similar arguments to \Cref{lem:whexp} can be found in the work of
...
The proof of \Cref{lem:app} relies
...
\begin{proof}
  ...
  by \Cref{lem:whexp}.  To this end, we have to show that all
...
In the appendix (\Cref{sec:sat}), we investigate how
the SN-method of
