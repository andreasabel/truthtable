...
connective, if-then-else, and the optimized inference rules for
...
ITTND have an arbitrary target.  In such settings, subsequent eliminations
...
the parallel simply-typed lambda calculus (parallel STLC), which is
similar to STLC with explicit non-determinism.
...
method, \emph{biorthogonals}, that have been successfully applied by
...
scale immeditately to the general case.
...
\par
  ...
  so-called de Bruijn index.  For example, de Bruijn index zero $\x_0$
