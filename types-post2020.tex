...
\Gamma$ and composition $\_\circ\_ : (\Gamma \leq \Delta) \to (\Delta
\leq \Phi) \to (\Gamma \leq \Phi)$.
If $a : \Gamma \vdash A$ then
\emph{weakening} $a\tau : \Delta \vdash A$ is defined in the usual way.
In particular, $\Up$ is used to traverse under binders.
...
Bruijn indices.
Substitutions form a category, and we reuse $\tid$ and $\_\circ\_$ for
...
The ``set'' of strongly normalizing terms $\SN$ given inductively by rule
\[
