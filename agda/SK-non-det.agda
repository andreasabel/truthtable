...
--
-- Further, there is a constructor ⊕ for non-deterministic choice.
...
⦅S⦆ {b = b} ⦅t⦆ (acc snt) ⦅u⦆ (acc snu) ⦅v⦆ (acc snv) = ty-cr b .cr3 Stuv λ where
  ...
  (↦∙l (↦∙l (↦∙r rt)))  → ⦅S⦆  (ty-cr _ .cr2 ⦅t⦆ rt) (snt rt)
                               ⦅u⦆ (acc snu)
                               ⦅v⦆ (acc snv)
  (↦∙l (↦∙r ru))        → ⦅S⦆  ⦅t⦆ (acc snt)
...
⦅K⦆ {a} ⦅t⦆ (acc snt) (acc snu) = ty-cr a .cr3 Ktu λ where
  ↦K              → ⦅t⦆
  (↦∙l (↦∙r rt))  → ⦅K⦆ (ty-cr a .cr2 ⦅t⦆ rt) (snt rt) (acc snu)
  (↦∙r ru)        → ⦅K⦆ ⦅t⦆ (acc snt) (snu ru)

-- Interpretation of choice: ditto.

⦅⊕⦆ : ⟦ a ⟧ t → SN t → ⟦ a ⟧ u → SN u → ⟦ a ⟧ (t ⊕ u)
⦅⊕⦆ {a} ⦅t⦆ (acc snt) ⦅u⦆ (acc snu) = ty-cr a .cr3 t⊕u λ where
  ↦l       → ⦅t⦆
  ↦r       → ⦅u⦆
  (↦⊕l r)  → ⦅⊕⦆ (ty-cr a .cr2 ⦅t⦆ r) (snt r) ⦅u⦆ (acc snu)
  (↦⊕r r)  → ⦅⊕⦆ ⦅t⦆ (acc snt) (ty-cr a .cr2 ⦅u⦆ r) (snu r)
